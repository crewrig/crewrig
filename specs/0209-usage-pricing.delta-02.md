...
1. **New requirement (R51) — A failed conversion keeps its USD amount,
   ...
   fixing exists on or before the computation date (requirement 22), or
   because the fixing used lists no rate for the requested currency
   (requirement 26) — the price SHALL keep its amount in USD, SHALL state
   USD as the currency that amount is denominated in, and SHALL carry a
   conversion status naming which of the two failures occurred and which
   currency was requested. No price SHALL state, as the currency of its
...
2. **New requirement (R52) — A failed conversion is re-attempted, never
   ...
   served from the store in answer to any later request, whatever currency
   that request names; the price SHALL be computed again, so that a request
   made after a fixing has become available — for instance after an
   explicit exchange-rate refresh — receives the converted price rather
   than the earlier failure. This applies equally to a stored price written
   before this delta, whose amount is USD but whose stated currency is the
   one requested.
...
6. **New requirement (R56) — Continuous-integration acceptance criterion
   ...
   currency at a computation date on or before which the fixture declares
   no fixing, each keep their USD amount labelled USD and carry the failure
   status of requirement 51; that a rollup of prices in such a currency,
   ...
   dashboard's for the same period, selection, currency, fixing date, and
   price-list snapshot, as requirement 47 requires; and that, once the
   missing fixture fixing is made available, a second request for the same
   record and currency receives the converted price rather than the stored
   failure, while a request in USD for a record whose stored price is a
   failure receives a price carrying no failed conversion status.
...
- Refusing a currency outright — failing the whole rollup because one
  record's conversion failed — is not a permitted outcome: the rollup
  reports its sums over the converted records and tallies the rest.
- Agreement with spec 0210's dashboard for `per-request` and `run-total`
  records stays outside requirement 47, as before this delta; this delta
...
Then  the price states an amount of 0.42 in USD, carries a conversion
      status naming the missing currency, and states no other currency as
      the denomination of its amount
```

**Scenario:** A stored failure is not served after the fixing arrives

```text
Given a record whose EUR price was stored after its conversion failed for
      want of any fixing on or before the computation date
...
currency, fixing date, and price-list snapshot, a rollup of prices over a
period SHALL report, for its `session-cumulative` records, the same price
...
**R52 scope — why "any later request".** The price store keeps one price
per record, whatever currency it was computed in. A stored failure
labelled USD under R51 would otherwise satisfy a later USD request's
currency check and carry its failure status into a USD rollup, where R54
would count as unconverted a record that needs no conversion at all. R52
therefore refuses a stored failure for every currency, and R51's last
sentence makes the USD case explicit. The same wording lets a price stored
