...
| `raw` | Subset | Per-CLI allow-list of top-level envelope keys, each copied unaltered when present: Antigravity `conversation_id`, `status`, `duration_seconds`, `num_turns`, `usage`; Claude Code `session_id`, `modelUsage`, `usage`, `total_cost_usd`, `duration_ms`, `num_turns`; Gemini CLI `session_id` and `stats` reduced to `stats.models`; Copilot CLI `currentModel`, `modelMetrics`; any other CLI `session_id`, `sessionId`, `model`, `usage`. Every CLI also keeps `timestamp` and `terminal_timestamp` | Never `response`, `result`, or other conversation text (spec 0206 R18) |
| `rawStatus` | Static | `"complete"` |  |
