...
**Whole-tree git operations need an exclusive claim over a clean tree.** `git reset --hard`, `git checkout -- .`, `git stash`, and `git clean` — and every other operation that discards, reverts, or relocates uncommitted changes across a whole tree rather than a named set of files — SHALL NOT run inside `.worktrees/<ticket-id>/` unless the acting agent holds the claim below for the **whole duration** of the operation and `git status --porcelain --untracked-files=all` is empty. The claim never waives the clean-tree condition: git records no author for an uncommitted change, so an empty status is the only proof that nothing you did not author is at risk. Their usual motive is verification: use the recipes below instead.
